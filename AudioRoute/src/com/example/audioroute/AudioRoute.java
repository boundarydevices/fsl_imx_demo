...
public class AudioRoute extends Activity {
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        if (fileHdmi.exists() && !fileSpdif.exists()) {
            ...
            mCommand.setText(getResources().getString(R.string.device_choose));
            mSpdifIn.setText(getResources().getString(R.string.Enable_SPDIF_In));
        } else Log.d(TAG, "no device connected");
        ...
        if (fileHdmi.exists() && !fileSpdif.exists()) {
            ...
            if (value.equals("2000")) mCheckBoxPassthrough.setChecked(true);
            else mCheckBoxPassthrough.setChecked(false);

            mCheckBoxPassthrough.setOnClickListener(
                    new OnClickListener() {
                        public void onClick(View v) {
                            Log.d(TAG, "CheckBox Passthrough is clicked");
                            SystemProperties.set(
                                    "vendor.persist.audio.pass.through",
                                    mCheckBoxPassthrough.isChecked() ? "2000" : "0");
                        }
                    });
        } else if (!fileHdmi.exists() && fileSpdif.exists()) {
            if (AudioSystem.getDeviceConnectionState(AudioSystem.DEVICE_IN_AUX_DIGITAL, "")
                    == AudioSystem.DEVICE_STATE_AVAILABLE) mSpdifIn.setChecked(true);
            else mSpdifIn.setChecked(false);

            mSpdifIn.setOnClickListener(
                    new OnClickListener() {
                        public void onClick(View v) {
        ...
        } else Log.d(TAG, "no device connected and can't do some operation");
    }

    private void setupAudioRoute() {
        Log.d(TAG, "SPDIF IN " + mSpdifIn.isChecked());
        mAudioManager.setWiredDeviceConnectionState(
                AudioSystem.DEVICE_IN_AUX_DIGITAL, mSpdifIn.isChecked() ? 1 : 0, "", "");
    }
}
