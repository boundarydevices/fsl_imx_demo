...
public class AudioTrackWrapper {
    ...
    public void init() {
        ...
        if (mChannel == AudioFormat.CHANNEL_CONFIGURATION_MONO) mChanCount = 1;
        else if (mChannel == AudioFormat.CHANNEL_CONFIGURATION_STEREO) mChanCount = 2;
        else System.out.println("mChanCount is wrong");

        if (mSampBit == AudioFormat.ENCODING_PCM_8BIT) mSampleBits = 1;
        else if (mSampBit == AudioFormat.ENCODING_PCM_16BIT) mSampleBits = 2;
        else System.out.println("mSampleBits is wrong");
    ...
    public void playAudioTrack(byte[] data, int offset, int length) {
        ...
        // System.out.println("-----------no any data, cur played frams
        // ---------"+mAudioTimestamp.framePosition);
        ...
        try {
            ...
            if (curFeedFrames < mAudioTimestamp.framePosition) {
                ...
                System.out.println(
                        "----------- cur played frams ---------" + mAudioTimestamp.framePosition);
            }
            ...
            if (deltatimeMs < min_deltatimeMs) min_deltatimeMs = deltatimeMs;
            if (deltatimeMs > max_deltatimeMs) max_deltatimeMs = deltatimeMs;
            sum += deltatimeMs;
            i++;
        } catch (Exception e) {
