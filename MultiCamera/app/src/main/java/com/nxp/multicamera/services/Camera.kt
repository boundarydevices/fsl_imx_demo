package com.nxp.multicamera.services
...
import com.nxp.multicamera.extensions.getCaptureSize
import com.nxp.multicamera.extensions.getPreviewSize
import com.nxp.multicamera.extensions.isAutoExposureSupported
import com.nxp.multicamera.extensions.isContinuousAutoFocusSupported
...
class Camera constructor(private val cameraManager: CameraManager) {
    ...
    var physicalCameraIdUsed = ArrayList<String>()

    var isLogicalCamera: Boolean =false

    lateinit var allCameraIds :Array<String>
    ...
    fun start(surfaces: List<Surface>) {
        ...
        if(surfaces.size > 1) {
            Log.e(TAG, "======startDualCamera======")
            startDualCamera(surfaces)
        } else {
            Log.e(TAG, "======startSingleCamera======")
    ...
    fun getSingleCameraIds(): String = cameraId

    fun getPhyCameraIds(): List<String> = physicalCameraIdUsed
    ...
    private fun startDualCamera(surfaces: List<Surface>) {
        val outputConfigs =  surfaces.mapIndexed { index, surface ->
            ...
            Log.e(TAG, "the physical camera id is:  $physicalCameraId")
            physicalCameraIdUsed.add(physicalCameraId)
    ...
    fun setUpCameraId(manager: CameraManager): String {
        allCameraIds = manager.cameraIdList;
        Log.e(TAG, "===All available cameras: " + Arrays.toString(allCameraIds))
        ...
        try {
            for (cameraId in manager.cameraIdList) {
                ...
                Log.e(TAG, "====$cameraId  isLogicalCamera?:  ${isLogicalCamera}===")
    ...
    private fun calculateZoomSize(manager: CameraManager) {
        if (this::physicalCameraIds.isInitialized) {
            physicalCameraIds.forEach {
                val characteristics = manager.getCameraCharacteristics(it)
                Log.d(
                    TAG,
                    "==== zoom $it ${
                        characteristics.get(SCALER_AVAILABLE_MAX_DIGITAL_ZOOM)?.toDouble()
                    }"
                )
            }
        }
    ...
    private fun calculateActiveArraySize(manager: CameraManager) {
        if (this::physicalCameraIds.isInitialized) {
            physicalCameraIds.forEach {
                ...
                Log.d(
                    TAG,
                    "==== width ${activeArraySize.width()} height ${activeArraySize.height()} $it"
                )
            }
        }
    }
    ...
    private fun setCropRegion(builder: CaptureRequest.Builder?, zoom: Double) {
        builder?.let {
            ...
            Log.d(
                TAG,
                "crop region(left=$left, top=$top, right=${left + width}, bottom=${top + height}) zoom($zoom)"
            )
