...
public class OtaAppActivity extends Activity implements OTAServerManager.OTAStateChangeListener {
    ...
    OnClickListener mDiffUpgradeListener =
            new OnClickListener() {
                public void onClick(View v) {
    ...
    OnClickListener mRebootListener =
            new OnClickListener() {
                public void onClick(View v) {
                    ...
                    new Runnable() {
                        public void run() {
                            mMessageTextView.setText(
                                    getText(R.string.error_package_install_failed));
                        }
                    });
        }
    }
    ...
    void onStateInChecked(int error, Object info) {
        ...
        if (error == 0) {
            // return no error, usually means have a version info from remote server, release name
            // is in @info
                        ...
                        new Runnable() {
                            public void run() {
                                ...
                                if (mOTAManager.compareLocalVersionToServer()
                                        == OTAServerManager.OtaTypeSelect.BOTH_OTA) {
                                    mUpgradeButton.setEnabled(true);
                                    mDiffUpgradeButton.setEnabled(true);
                                } else if (mOTAManager.compareLocalVersionToServer()
                                        == OTAServerManager.OtaTypeSelect.FULL_OTA) {
                                    mUpgradeButton.setEnabled(true);
                                    mDiffUpgradeButton.setEnabled(false);
                                } else if (mOTAManager.compareLocalVersionToServer()
                                        == OTAServerManager.OtaTypeSelect.DIFF_OTA) {
