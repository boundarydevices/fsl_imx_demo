...
public class OTAServerConfig {
    ...
    public OTAServerConfig(String productname) throws MalformedURLException {
        if (loadConfigureFromFile(configFile, productname) == false) defaultConfigure(productname);
    }
    ...
    boolean loadConfigureFromFile(String configFile, String product) {
        try {
            ...
            if (!ab_slot) {
                ...
                diffPayloadPropertiesURL =
                        new URL(default_protocol, server, port, diffPayloadPropertiesAddr);
    ...
    public URL getPackageURL() {
        if (ab_slot) {
            if (getDiffUpgrade()) return diffPayloadURL;
            else return payloadURL;
        } else return updatePackageURL;
    }

    public URL getPayloadPropertiesURL() {
        if (getDiffUpgrade()) return diffPayloadPropertiesURL;
        else return payloadPropertiesURL;
    }
