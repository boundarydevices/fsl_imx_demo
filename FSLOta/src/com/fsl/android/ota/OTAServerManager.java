...
public class OTAServerManager {
        ...
        public void onPayloadApplicationComplete(int errorCode) {
            ...
            if (errorCode == UpdateEngine.ErrorCodeConstants.SUCCESS) {
                if (this.mListener != null && !mStop)
                    this.mListener.onStateOrProgress(
                            OTAStateChangeListener.MESSAGE_WAIT_REBOOT, 0, null);
            }
        }
    }
    ...
    public void startCheckingVersion() {
        ...
        if (parser != null) {
            if (this.mListener != null)
                this.mListener.onStateOrProgress(
                        OTAStateChangeListener.STATE_IN_CHECKED,
                        OTAStateChangeListener.NO_ERROR,
                        parser);
        } else {
    ...
    public OtaTypeSelect compareLocalVersionToServer() {
        ...
        if (buildutc < remoteBuildUTC) full_ota = true;
        if (ab_slot()) {
            remoteDiffBuildUTC = (Long.parseLong(parser_diff.getProp("ro.build.date.utc"))) * 1000;
            remoteDiffBuildBaseUTC =
                    (Long.parseLong(parser_diff.getProp("base.ro.build.date.utc"))) * 1000;
            if (buildutc < remoteDiffBuildUTC && buildutc.equals(remoteDiffBuildBaseUTC))
                diff_ota = true;
        }
        if (full_ota && diff_ota) upgrade = OtaTypeSelect.BOTH_OTA;
        else if (full_ota) upgrade = OtaTypeSelect.FULL_OTA;
        else if (diff_ota) upgrade = OtaTypeSelect.DIFF_OTA;
        // here only check build time, in your case, you may also check build id, etc.
        Log.d(
                TAG,
                "remote BUILD TIME: "
                        + remoteBuildUTC
                        + " remote DIFF BUILD TIME: "
                        + remoteDiffBuildUTC
                        + " remote DIFF BUILD BASE TIME: "
                        + remoteDiffBuildBaseUTC
                        + " local BUILD TIME:"
                        + buildutc);
        return upgrade;
    ...
    void publishDownloadProgress(long total, long downloaded) {
        ...
        if (this.mListener != null && progress.longValue() != mCacheProgress) {
            this.mListener.onStateOrProgress(
                    OTAStateChangeListener.MESSAGE_DOWNLOAD_PROGRESS, 0, progress);
    ...
    void reportDownloadError(int error) {
        if (this.mListener != null)
            this.mListener.onStateOrProgress(
                    OTAStateChangeListener.STATE_IN_DOWNLOADING, error, null);
    }

    void reportInstallError(int error) {
        if (this.mListener != null) {
            this.mListener.onStateOrProgress(
                    OTAStateChangeListener.STATE_IN_UPGRADING, error, null);
    ...
    public void startDownloadUpgradePackage() {
        ...
        try {
            ...
            if (this.mListener != null && !mStop)
                this.mListener.onStateOrProgress(
                        OTAStateChangeListener.STATE_IN_DOWNLOADING, 0, null);
        } catch (IOException e) {
            e.printStackTrace();
            reportDownloadError(OTAStateChangeListener.ERROR_WRITE_FILE_ERROR);
        } finally {
    ...
    RecoverySystem.ProgressListener recoveryVerifyListener =
            new RecoverySystem.ProgressListener() {
                public void onProgress(int progress) {
                    Log.d(TAG, "verify progress" + progress);
                    if (mListener != null)
                        mListener.onStateOrProgress(
                                OTAStateChangeListener.MESSAGE_VERIFY_PROGRESS,
                                0,
                                new Long(progress));
                }
            };
    ...
    BuildPropParser getTargetPackagePropertyList(URL configURL) {
        ...
        try {
            ...
            Log.d(
                    TAG,
                    "download finish:" + (new Integer(totalBufRead).toString()) + "bytes download");
            reader.close();
