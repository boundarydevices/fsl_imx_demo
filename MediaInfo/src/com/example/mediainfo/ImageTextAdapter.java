...
public class ImageTextAdapter extends BaseAdapter {
    ...
    public ImageTextAdapter(Context c) {
        ...
        int i;
        for (i = 0; i < mBuckets.length; i++) mBuckets[i] = new Bucket();
        mBucketCount = 0;
        mCurrentBucket = -1;
    }

    public synchronized int getCount() {
        if (mCurrentBucket == -1) return mBucketCount;
        else return mBuckets[mCurrentBucket].length();
    }
    ...
    public synchronized View getView(int position, View convertView, ViewGroup parent) {
        ...
        if (itv == null) itv = new ImageTextView(mContext);
        if (mCurrentBucket == -1 && position >= 0 && position < mBucketCount)
            itv.update(mBuckets[position].name(), mBuckets[position].length(), position);
        else if (mCurrentBucket >= 0
                && mCurrentBucket < mBucketCount
                && position >= 0
                && position < mBuckets[mCurrentBucket].length())
    ...
    private void createBuckets(ItemData[] data, int count) {
        ...
        for (i = 0; i < count; i++) {
            ...
            for (j = 0; j < mBucketCount; j++) {
                if (mBuckets[j].name().equals(bucketName)) break;
            }
            ...
            if (j >= mBuckets.length) {
                ...
                for (; k < newBuckets.length; k++) newBuckets[k] = new Bucket();
            ...
            if (j == mBucketCount) mBucketCount++;
        }
    }

    public synchronized void setData(ItemData[] data, int count) {
    ...
    private class Bucket {
        ...
        public void insert(ItemData data) {
            ...
            if (mName == null) mName = data.mBucket;
